...
namespace teensy_serial_demo
{
    public partial class Form1 : Form
    {
        ...
        {
            // parse individual bytes from the string
            const byte motorPacketDescriptor = 0x06;
            byte[] payloadBuffer = { motorPacketDescriptor, Convert.ToByte(trackBar1.Value), Convert.ToByte(checkBox2.Checked), Convert.ToByte(checkBox3.Checked) };
            ...
            packetBuffer[0] = 0xAA;
            packetBuffer[1] = Convert.ToByte(packetLength);
            packetBuffer[2] = payloadBuffer[0];
            packetBuffer[3] = payloadBuffer[1];
            packetBuffer[4] = payloadBuffer[2];
            packetBuffer[5] = payloadBuffer[3];
            byte checkSum = Convert.ToByte(packetBuffer[0] ^ packetBuffer[1]);
